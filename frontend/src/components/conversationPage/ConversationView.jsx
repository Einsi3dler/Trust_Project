...
export default function ConversationView ({ userId, receiverId, currentChatUserName="You" }) {
  ...
  <div className="container-fluid">
    ...
    <span className="navbar-brand user-name text-dark text-capitalize ">{currentChatUserName}</span>
  </div>
</nav>
<div className="msg-box container-fluid d-flex flex-column-reverse" id="msgbox">
        {allMessage &&
          allMessage.map((item, index) => (
            <div
              key={index}
              className= {`text-dark message d-flex
			  ${item.receiver_id === userId ? "justify-content-start" : "justify-content-end"}`}
            >
                    <p className={`text-break ${item.receiver_id === userId ? "text-start" : "text-end"} `}>{item.message}</p>
                </div>
              ))}
    ...
    <form onSubmit={ onSubmit }>
      <input  onChange={ e => setValue(e.target.value) } className="message-input overflow-auto" type="text" id="text" placeholder="Enter your message..." required/>
