...
export default function ConversationPage ({ user }) {
  ...
  useMemo(() => {
	...
	function startConversation (event, receiver_id, chatUserFirstName, chatUserLastName="") {
		...
		const check = window.getComputedStyle(responsivePage).getPropertyValue('display');
		if (check === "none" && currentReceiver) {
			 navigate("/responsive", {state: { userId: user.id, receiverId: currentReceiver, currentChatUserName: currentChatUsername}});
		}
	}
  ...
  <div className="container-fluid">
	...
	<Navbar.Toggle aria-controls="basic-navbar-nav" onClick={handleShow} className="m-1" />
			<Offcanvas
            ...
            <Offcanvas.Title id="navbar-label">
              ...
              <Offcanvas.Body>
         	 <div className="chat-menu justify-content-end flex-grow-1 pe-3" id="nav-menu">
				<ul>
				<li><a href="/">Dashboard</a></li>
				<li><a href="/">Transactions</a></li>
				<li><a href="/">Buyers</a></li>
				<li><a href="/">Sellers</a></li>
				<li><a href="/">Accounts Setting</a></li>
				<li className="chat-logout"><button className=" btn btn-primary col-4"><a href="/logout">Log out</a></button></li>
				</ul>
			</div>
			</Offcanvas.Body>
			</Offcanvas>
		...
		<form action="">
			<input type="text" className="search-input bg-dark" placeholder=" Search messages"/>
		</form>
		</div>
		{spokenTo?.map((el, index) =>
		<section className="chattedUser d-flex" key={index} onClick={ (e) => startConversation(e, el.receiverId, el.spokeToUserFirstName, el.spokeToUserLastName) } id={el.receiverId}>
			<svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" className="bi bi-person-circle" viewBox="0 0 16 16">
			<path d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0z"/>
			<path fillRule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1z"/>
			</svg>
		<p className="user-name text-capitalize text-start">{el.spokeToUserFirstName} {el.spokeToUserLastName} </p>
		</section>
		)}
		</aside>
		<main className="view d-none d-md-block" id="pageresponsive">
			<ConversationView  userId={ user.id} receiverId={currentReceiver}  currentChatUserName={currentChatUsername}>
			</ConversationView>
		</main>
		</Container>
		</>
