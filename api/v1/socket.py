...
def send_msg(data):
    ...
    receiver = list(storage.get(User, data['receiver_id']).values())[0]
    conversation_data = {"message": data['message'], "sender_id": user.id, "receiver_id": receiver.id}
    conversation = Conversation(**conversation_data)
    conversation.save()
    user.conversations.append(conversation)
    receiver.conversations.append(conversation)
    user.save()
    receiver.save()
    print(data['message'])
    receiver_sid = find_receiver_sid(data['receiver_id'])
    emit("receive_msg", {"message": conversation.message, "receiver_id": data['receiver_id'], "sender_id": data['sender_id']}, to=receiver_sid)
